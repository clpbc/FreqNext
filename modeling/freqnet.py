...
class FreqNet(nn.Module):
    ...
    def __init__(self, block=Bottleneck, layers=[3, 4], num_classes = 2, zero_init_residual=False):
    ...
    def forward(self, x):
        ...
        imag_part = self.imagconv1(x.imag).to(torch.float32)
        ...
        x = torch.fft.fft2(x, norm="ortho")#,norm='forward'
        x = torch.fft.fftshift(x, dim=[-2, -1]) 
        real_part = self.realconv2(x.real).to(torch.float32)
        imag_part = self.imagconv2(x.imag).to(torch.float32)
        x = torch.complex(real_part, imag_part)
        x = torch.fft.ifftshift(x, dim=[-2, -1])
        x = torch.fft.ifft2(x, norm="ortho")
        x = torch.real(x)
        ...
        x = torch.fft.fft2(x, norm="ortho")#,norm='forward'
        x = torch.fft.fftshift(x, dim=[-2, -1]) 
        real_part = self.realconv3(x.real).to(torch.float32)
        imag_part = self.imagconv3(x.imag).to(torch.float32)
        x = torch.complex(real_part, imag_part)
        x = torch.fft.ifftshift(x, dim=[-2, -1])
        x = torch.fft.ifft2(x, norm="ortho")
        x = torch.real(x)
        ...
        x = torch.fft.fft2(x, norm="ortho")#,norm='forward'
        x = torch.fft.fftshift(x, dim=[-2, -1]) 
        real_part = self.realconv4(x.real).to(torch.float32)
        imag_part = self.imagconv4(x.imag).to(torch.float32)
        x = torch.complex(real_part, imag_part)
        x = torch.fft.ifftshift(x, dim=[-2, -1])
        x = torch.fft.ifft2(x, norm="ortho")
        x = torch.real(x)
