...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if exists(attn_mask):
            ...
            dots = dots.masked_fill(~attn_mask, -1e9)
...
class NaViT(nn.Module):
    def __init__(self, cfg):
        ...
        self.model = navit(
            ...
            dim = self.cfg['model']['embed_dim'],
            depth = self.cfg['model']['depth'],
            heads = self.cfg['model']['heads'],
            mlp_dim = self.cfg['model']['mlp_ratio'] * self.cfg['model']['embed_dim'],
            channels = self.cfg['model']['in_channels'],
            dim_head = self.cfg['model']['embed_dim'] // self.cfg['model']['heads'],
            dropout = self.cfg['model']['dropout'],
            emb_dropout = self.cfg['model']['embed_dropout'],
            token_dropout_prob = self.cfg['model']['token_dropout_prob'],
        )
        
        self.group_max_seq_len = self.cfg['model']['group_max_seq_len']
    ...
    def forward(self, input_dict):
        ...
        logits = self.model(
            img,
            group_images = True,
            group_max_seq_len = self.group_max_seq_len
        )
