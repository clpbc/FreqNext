...
class DWT_2D(nn.Module):
    def __init__(self, wave):
        ...
        w = pywt.Wavelet(wave)  # 创建指定小波基的Wavelet('haar')对象
        dec_hi = torch.Tensor(w.dec_hi[::-1])   # dec_hi为高通滤波器系数,[:: -1]将滤波器系数反转（PS：卷积中顺序与小波定义相反）
        dec_lo = torch.Tensor(w.dec_lo[::-1])   # dec_lo为低通滤波器系数
